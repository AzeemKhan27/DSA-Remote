...
public:
    void solve(vector<string> &ans, int n, int used_open, int used_close,int rem_open,int rem_close, string output){
        ...
        if(rem_open > 0){
            // output.push_back('(');
            solve(ans, n, used_open + 1, used_close, rem_open - 1, rem_close, output + '(');  //recursive call
        ...
        if(used_open > used_close){
            // output.push_back(')');
            solve(ans, n, used_open, used_close + 1, rem_open, rem_close - 1, output + ')');    //recursive call
            // output.pop_back();
        }
